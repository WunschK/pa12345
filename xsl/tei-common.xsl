<!-- W.DB+ generic XSLTs – common templates and functions
     ...
     functions/templates can be overwritten --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:html="http://www.w3.org/1999/xhtml" exclude-result-prefixes="#all" version="3.0">
...
<!-- !!!!! CHANGE !!!!! implement changes to view outline as created in branch accessibility-view-outline
  ...
  <xsl:template match="tei:text">
    ...
    <article>
      <xsl:if test="//tei:note[@place = 'margin']">
        <section aria-label="contains marginalia of an original text or side notes of a digital text" id="marginalia_container">
  ...
  <xsl:template match="tei:rs">
    ...
    <xsl:choose>
      ...
      <xsl:when test="tei:subst or tei:choice or tei:app">
        ...
        <xsl:variable name="appElement" as="element()" select="tei:subst | tei:choice | tei:app"/>
        <xsl:variable name="appValue" as="element()" select="$appElement/tei:add | $appElement/tei:corr[1] | $appElement/tei:lem"/>
  ...
  <xsl:template match="tei:pb">
    ...
    <xsl:choose>
      <xsl:when test="@facs">
        ...
        <button aria-label="a pagebreak with a link to a facsimile" class="pagebreak" id="p{@ed}-{@n}" data-image="{$image}">
  ...
  <xsl:template match="tei:ptr[@type = 'wdb']">
    ...
    <a class="crossRef" href="view.html?id={$file/tei:TEI/@xml:id}{if($values[2]) then '#' || $values[2] else ''}" aria-label="link to a place within this file or another file in the framework">
  ...
  <xsl:template match="tei:*" mode="fnLink">
    ...
    <button id="{$position}{$type}{$number}" data-note="{$type}{$number}" class="footnoteNumber" aria-label="opens a footnote">
  ...
  <xsl:template match="*" mode="fnumberAlph">
    <xsl:number format="a" level="any" count="tei:choice       | tei:app[not(ancestor::tei:choice)]       | tei:subst       | tei:add[not(parent::tei:subst | parent::tei:lem | parent::tei:rdg)]       | tei:del[not(parent::tei:subst | parent::tei:lem | parent::tei:rdg)]       | tei:note[@type='crit_app']       | tei:seg[@hand or @resp]       | tei:unclear[@extent]"/>
  </xsl:template>
